source p.sh
filename="$1"
readarray X < <(grep  -n "^{" $filename|grep  {|cut -d ":" -f1)
readarray Y < <(grep  -n "^}" $filename|grep  }|cut -d ":" -f1)
for ((i=1;i<=${#X[@]};i++));do
...
#echo $text_in_keys
text_index=$(echo ${keys[@]}|grep -oE ".*?text"|grep -o ' '|wc -l)
...
pg "insert into hn_stories ($(echo ${keys[@]}|tr ' ' ',')) values ('$(echo ${values[@]}|sed -re "s/, /','/g"|tr '][' '}{')') ";
